...
AM002 does not report when the destination member is explicitly configured to handle nulls, such as `MapFrom(src => src.Name ?? "fallback")`, a safe AutoMapper `NullSubstitute("fallback")`, an assignable `NullSubstitute` fallback such as a `string` value for an `object` destination member, a typed value-type default such as `NullSubstitute(default(int))`, stable-value guarded nullable dereferences such as `src.Name == null ? string.Empty : src.Name.Trim()`, `src.Name == default ? string.Empty : src.Name.Trim()`, guarded conditional results such as `MapFrom(src => (src.Name != null ? src.Name : string.Empty)!)`, outer fallback coalescing such as `MapFrom(src => (src.Name ?? default!) ?? string.Empty)`, unreachable coalesce fallbacks behind non-nullable or stable-guarded left values such as `MapFrom(src => (src.Child ?? src.FallbackChild!).Name)`, non-null guards and typed value-default equality true-branch guards such as `MapFrom(src => src.Name == "special" ? src.Name! : string.Empty)` and `MapFrom(src => src.Count == default(int) ? src.Count!.Value : 0)`, null-forgiven guard comparisons on stable values such as `MapFrom(src => src.Name! != null ? src.Name! : string.Empty)`, short-circuit guards such as `MapFrom(src => src.Name != null && src.Name!.Length > 0 ? src.Name : string.Empty)`, negated null-check guards such as `MapFrom((src, dest) => !(src.Name == null) ? src.Name!.Length : 0)`, switch guards such as `MapFrom((src, dest) => src.Name switch { null => string.Empty, _ => src.Name! })`, or delegate `MapFrom((src, dest) => src.Name is not null and { Length: > 0 } ? src.Name!.Trim() : string.Empty)`, nullable value defaults such as `src.Count.GetValueOrDefault()`, AutoMapper `Ignore()`, a proven non-null-producing resolver expression that does not dereference a nullable receiver unsafely, a custom resolver form such as `MapFrom<TResolver>()` or `MapFrom<TResolver, TSourceMember>(...)`, or a member-level value converter such as `ConvertUsing<TConverter, TSourceMember>(...)`. Pass-through, null-forgiving pass-throughs or dereferences, repeated nullable-returning method calls guarded only by a previous invocation, typed value-default comparison branches that null can still reach, unguarded nullable-receiver dereferences, different-member, and generic expression mappings like `MapFrom(src => src.Name)`, `MapFrom(src => src.Name!)`, `MapFrom(src => (string)src.Name!)`, `MapFrom(src => (object)src.Name!)`, `MapFrom(src => (src.Name ?? src.OtherName)!)`, `MapFrom(src => src.Name! ?? src.OtherName!)`, `MapFrom(src => src.Name ?? src.OtherName!)`, `MapFrom(src => src.Name ?? default!)`, `MapFrom(src => (src.Name ?? default!)!)`, `MapFrom(src => (src.Name ?? null!)!)`, `MapFrom(src => (src.Name ?? src.OtherName!).Length)`, `MapFrom(src => (src.Name ?? null)!)`, `MapFrom(src => (src.Name ?? default)!)`, `MapFrom((src, dest) => src.Name?.Trim()!)`, `MapFrom(src => (src.Name != null ? src.Name : null)!)`, `MapFrom(src => (src.UseOther ? src.OtherName : string.Empty)!)`, `MapFrom((src, dest) => (src.UseName switch { true => src.Name, _ => string.Empty })!)`, `MapFrom((src, dest) => src.UseOther switch { true => src.OtherName!, _ => string.Empty })`, `MapFrom(src => (src.UseName ? src.Name! : string.Empty).Length)`, `MapFrom(src => (string?)null!)`, `MapFrom(src => default(string)!)`, `MapFrom(src => (src.Value as string)!)`, `MapFrom(src => src.Values[0]!)`, `MapFrom(src => src.Values![0])`, `MapFrom(src => src.Name!.Length)`, `MapFrom(src => ((string)src.Name!).Length)`, `MapFrom(src => src.Factory!())`, `MapFrom(src => src.Name.Trim())`, `MapFrom(src => src.Name.Length)`, `MapFrom(src => src.OtherNullableName)`, and `MapFrom<TSourceMember>(src => src.Name)` still report when the mapped value can come from a nullable source and the destination member is non-nullable, and diagnostics name the actual nullable source member or non-source expression used by the explicit mapping. Unsafe substitutes such as `NullSubstitute(null)`, `NullSubstitute(default)`, and nullable/reference typed defaults still report. Helper methods named `Ignore`, `NullSubstitute`, or `MapFrom` are not treated as AutoMapper null-handling or mapping options unless they are invoked on the AutoMapper options parameter. AM002 also stays quiet when the map uses custom construction/conversion, when nullable reference annotations are disabled or oblivious, or when the nullable source and destination member have incompatible underlying types owned by `AM001`.
