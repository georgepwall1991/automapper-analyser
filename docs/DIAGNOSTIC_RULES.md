...
Semantic AutoMapper `ForCtorParam` configuration follows the same null-safety rules for the exact parameter selected by AutoMapper's longest resolvable destination constructor. Literal, `nameof(...)`, and const-string names must match ordinally; public readable source properties, inherited public fields, public parameterless method/`GetX()` conventions, sibling configuration, optional/defaulted parameters, and `params` can make constructor siblings resolvable. A non-null-producing constructor `MapFrom` suppresses AM002, while direct nullable, null-forgiving-only, unsafe receiver, and nullable-element expressions still report. Same-name members and uniquely proven read-only aliases remain constructor-owned. A direct-only writable alias delegates only when an unvetoed convention mapping or the last effective top-level `ForMember`/`ForPath` with unconditional `MapFrom`/`ConvertUsing` guarantees the final value; `Ignore`, `Condition`, and `PreCondition` leave the constructor input analyzed. The default-value fix updates the owning constructor expression only when it can emit a provably non-null fallback. Property Ignore is withheld whenever removing post-construction assignment would expose an unsafe constructor alias, but remains available for a proven non-null alias. Ownership stays scoped to the effective `ReverseMap()` direction; unmatched names and lookalike APIs do not suppress diagnostics. Directly executed map-wide `ForAllMembers` `Ignore`, `Condition`, and `PreCondition` policies veto both convention and targeted post-construction ownership because AutoMapper applies them after member configuration. Calls captured inside uninvoked lambdas or local functions remain deferred and do not veto assignment. Non-async local functions reached from the callback's synchronous execution region are followed semantically, including nested calls; source-ordered options alias state flows into each executed call, options-valued local-function arguments bind their corresponding parameters, nested helper calls expand child/argument invocations before the containing call to match C# evaluation order, definite and conditional receiver aliases are tracked separately, and reassignment removes may-alias provenance only when it is guaranteed on every path reaching the invocation; an active-call guard terminates direct and mutual recursion. Constructor `MapFrom` body selection consumes that same executed invocation stream, so options aliases and invoked helpers participate while uninvoked helpers and deferred delegates do not. Conditional mapping branches remain runtime alternatives unless a later unconditional `MapFrom` replaces them, preventing a safe textual last branch from hiding an unsafe path. Parenthesized option receivers retain semantic alias identity. Iterator bodies participate when the callback synchronously enumerates the invocation, materializes it through `System.Linq.Enumerable.ToList`/`ToArray`, or enumerates any semantic local target in its enclosing assignment chain that still holds the iterator after source-ordered assignments; per-member configuration, including `NullSubstitute`, uses the same deferred-scope filter.

AM002 recursively checks nested generic output and invariant nullability, but it does not interpret contravariant input arguments as produced values. Safe consumer mappings such as `Action<string?>` to `Action<string>`, including nested input types, therefore stay quiet instead of receiving a producer-oriented nullable-to-non-nullable diagnostic.

When the same destination member is configured more than once, AM002 evaluates and fixes the later effective configuration. The default-value fixer preserves existing top-level `ForMember` and top-level `ForPath` options when it adds null handling, emits fully qualified framework defaults such as `global::System.DateTime.MinValue`, and uses `default!` for generic/reference fallback defaults where plain `default` would remain nullable, but it does not reuse child `ForPath` mappings as top-level nullable-property fixes and it withholds the default-value action when existing `Condition` or `PreCondition` guards can veto assignment or when an existing `MapFrom` dereferences a nullable receiver before any fallback could run. When appending `MapFrom` to existing non-veto member options for a nullable method convention, the fixer preserves the analyzer-computed source invocation (for example `src.GetValue()`) instead of reconstructing a nonexistent property access. A single directly executed constructor `MapFrom` reached through an options alias or invoked helper is rewritten at its original argument, so transforms are preserved rather than overridden by a reconstructed mapping.
...
Registrations in opposite arms of one `if`/`else` chain or distinct sections of one `switch` are not
duplicates only inside a direct `MapperConfiguration` callback, where the configuration body runs once for
that container. The constructed container must come from the same AutoMapper assembly as the recognized
`CreateMap` calls; source-defined namespace lookalikes fail closed. `Profile` constructors remain diagnostic because multiple profile instances can select
different branches or sections in one configuration. Repeatable helpers, local functions, and ordinary
lambdas also fail closed and keep AM041 enabled, as does any loop surrounding the conditional or nested
around either registration, and any
...
1. **Check package reference**:
   ```xml
   <PackageReference Include="AutoMapperAnalyzer.Analyzers" Version="2.30.110">
