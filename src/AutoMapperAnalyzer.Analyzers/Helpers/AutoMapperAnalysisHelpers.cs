...
public static class AutoMapperAnalysisHelpers
{
        ...
        SemanticModel semanticModel)
    {
        ...
        {
            TypeInfo sourceTypeInfo = semanticModel.GetTypeInfo(genericName.TypeArgumentList.Arguments[0]);
            TypeInfo destTypeInfo = semanticModel.GetTypeInfo(genericName.TypeArgumentList.Arguments[1]);
        ...
        {
            TypeInfo sourceTypeInfo = semanticModel.GetTypeInfo(genericMemberName.TypeArgumentList.Arguments[0]);
            TypeInfo destTypeInfo = semanticModel.GetTypeInfo(genericMemberName.TypeArgumentList.Arguments[1]);
            return (sourceTypeInfo.Type, destTypeInfo.Type);
        }
    ...
    public static bool AreTypesCompatible(ITypeSymbol? sourceType, ITypeSymbol? destType)
    {
        ...
        if (IsNullableType(destType, out ITypeSymbol? destUnderlyingType) &&
    ...
    public static bool IsCollectionType(ITypeSymbol? type)
    {
        ...
        {
            INamedTypeSymbol? enumerableInterface = namedType.AllInterfaces
                .FirstOrDefault(i =>
    ...
    public static ITypeSymbol? GetCollectionElementType(ITypeSymbol? collectionType)
    {
        ...
        if (collectionType is INamedTypeSymbol namedType)
        {
            ...
            {
                string genericTypeDef = namedType.OriginalDefinition.ToDisplayString();
