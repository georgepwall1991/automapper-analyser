...
public class AM005_CaseSensitivityMismatchCodeFixProvider : AutoMapperCodeFixProviderBase
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        foreach (Diagnostic? diagnostic in context.Diagnostics)
        {
            ...
            InvocationExpressionSyntax? invocation = GetCreateMapInvocation(operationContext.Root, diagnostic, properties);
            if (invocation == null)
            {
                continue;
            }
            ...
            // Destination is escaped by CreateForMemberWithMapFrom; source is interpolated into a parsed expression.
            string escapedSource = CodeFixSyntaxHelper.EscapeIdentifier(sourcePropertyName);
